...
public class Plateau implements Serializable {
    ...
    //niveau venant du jeu de base
    public Plateau(int hauteur, int largeur, int animaux, int nbCoups, int score, int ballons, int bombes, int fusees, int marteaux) {
        this.scoreGagnant = score;
        Plateau.nbCoups = nbCoups;
        this.nbBallons = ballons;
        this.nbBombes = bombes;
        this.nbFusees = fusees;
        this.nbMarteaux = marteaux;
    ...
    public void affiche() {
        ...
        if (nbFusees != 0)
            System.out.println("Fusées : " + nbFusees);
        if (nbMarteaux != 0)
            System.out.println("Marteaux : " + nbMarteaux);
    ...
    public void ajouteObstacles(String type) {
        for (int i = 0; i < hauteur; i++) {
            for (int j = 0; j < largeur; j++) {
                if (cases[i][j] == null)
                    this.cases[i][j] = new BlocIncassable(type);
            }
        }
    }

    public void ajouteObstacles(String type, int h, int l) {
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < l; j++) {
                if (cases[i][j] == null)
                    this.cases[i][j] = new BlocIncassable(type);
        ...
        if (a - 1 >= 0) { //on verifie qu'on ne sorte pas du plateau
            if (cases[a - 1][b] instanceof Bloc && !(cases[a - 1][b] instanceof Animal)) { //on verifie que la case du dessus soit bien un Bloc
                if (couleur.equals(cases[a - 1][b].getCouleur()) && adjacent[a - 1][b] == null) { //on verifie que le Bloc au dessus soit de la meme couleur que le Bloc actuel
                    pareil = true;
                    adjacent[a - 1][b] = cases[a][b]; //on remplit le tableau contenant les blocs adjacents avec un Bloc de la meme couleur
        ...
        if (b - 1 >= 0) { //on verifie qu'on ne sorte pas du plateau
            if (cases[a][b - 1] instanceof Bloc) { //on verifie que la case de gauche soit bien un Bloc
                if (couleur.equals(cases[a][b - 1].getCouleur()) && adjacent[a][b - 1] == null) { //on verifie que le Bloc a gauche soit de la meme couleur que le Bloc actuel
                    pareil = true;
                    adjacent[a][b - 1] = cases[a][b]; //on remplit le tableau contenant les blocs adjacents avec un Bloc de la meme couleur
        ...
        if (cases[h][l] instanceof Bloc) {
            if (cases[h][l] instanceof BlocBonus) {
                ...
                if (!(cases[h][l] instanceof BlocIncassable)) {
                    if (memeCouleur(h, l) && !(cases[h][l] instanceof Animal)) {
                        for (int i = 0; i < hauteur; i++) {
                            for (int j = 0; j < largeur; j++) {
    ...
    //on verifie si une colonne contient au moins un BlocIncassable
    public boolean obstacle(int col) {
        for (int i = 0; i < hauteur; i++) {
            if ((cases[i][col] instanceof BlocIncassable)) {
                if (col+1 < largeur) {
                    if (cases[i][col+1] != null && !(cases[i][col+1] instanceof BlocIncassable))
                        return true;
                }
            }
        }
    ...
    public boolean gagne() {
        ...
        //si on a atteint un certain score et sauvé tous les animaux, on a gagné
        return score >= scoreGagnant && nbAnimaux == 0;
    }

    public boolean fini() {
        //s'il ne reste plus de coups, on a fini
        if (nbCoups == 0)
            return true;

        //s'il ne reste qu'un seul bloc, on a fini
        boolean vide = true;
        for (int j = 1; j < largeur; j++) {
            if (cases[hauteur - 1][j] != null) {
                vide = false;
                break;
            }
        }
        if (cases[hauteur-1][0] != null && vide)
            return true;

        //si le plateau est vide, on a fini
        if (cases[hauteur-1][0] == null)
            return true;
